var HOMELESSLINKSTARS = function() {
    ...
    var getStarLines = function(starName, RecordId, RecordIdOfParent, imagePath, relationshipField, dateField, scoreFields, 
		...
		//Get RecordType info from RecordId
		var recTypeName = (starName.indexOf('___') != -1)? starName.split('___')[1]: '';
		var recTypeId = (starName.indexOf('___') != -1)? starName.split('___')[2]: '';
        starName = (starName.indexOf('___') != -1)? starName.split('___')[0]: starName;
		// object to hold metadata
		var starMetaData = {};
		// the list of score fields from the page as an array
        ...
        var starRecords = [];
        getConfigDetailObjectRecords(remoteConfigDetailObject, queryMeta, starRecords, function(starRecords) {
            if (starRecords != null) {
				// populate the metadata with the default value for empty picklists
                processMetaData(scorefieldArray, starMetaData, starOrigin, starRecords, namespacePrefix);
                // retrieve the star records
				remoteStarObject.retrieve(queryRecords, function(err, records, event) {
                    if (err) {
                        handleErrors(err)
                    } else {
						// merge the metadata and star records into something usable by the front-end and pass this to makeStars
						if(recTypeName != '' && recTypeId != ''){
							var filterRecords = [];							
							for (var index = 0; index < records.length; ++index) {
								if(records[index]._props && records[index]._props.RecordTypeId == recTypeId){
									filterRecords.push(records[index]);
								}
							}							
							mergeStarData(filterRecords, starCurrentCoordinate, scorefieldArray, starMetaData, namespacePrefixObject, starLineData, colours, dateField, starOpacity, lineWidth);
							makeStars(starLineData, imagePath, RecordId)
						}else{
							mergeStarData(records, starCurrentCoordinate, scorefieldArray, starMetaData, namespacePrefixObject, starLineData, colours, dateField, starOpacity, lineWidth);
							makeStars(starLineData, imagePath, RecordId)
						}
                    }
                })
    ...
    var getConfigDetailObjectRecords = function(remoteConfigDetailObject, queryMeta, totalRecords, callback) {
        remoteConfigDetailObject.retrieve(queryMeta, function(err, records, event) {
            if (err) {
                handleErrors(err);
                callback(null)
            } else {
                if (records != null && records.length == 100) {
                    queryMeta.offset = (queryMeta.offset) ? queryMeta.offset : 0;
                    queryMeta.offset = queryMeta.offset + records.length;
                    totalRecords = totalRecords.concat(records);
                    if (queryMeta.offset != null && queryMeta.offset < 2000)
                        getConfigDetailObjectRecords(remoteConfigDetailObject, queryMeta, totalRecords, callback);
                    else callback(totalRecords)
                } else {
                    totalRecords = totalRecords.concat(records);
    ...
    var chooseColourOrGray = function(starColours, userGray, starGrays) {
        ...
        if (userGray === !0) retVal = starGrays.split(",");
        return retVal
    };
	
    var getlineWidth = function(starStrokeWidth, userWidth) {
        var retVal = starStrokeWidth;
        if (!isNaN(userWidth) && userWidth > 0) retVal = userWidth;
        return retVal
    };
	
	// convert undefined to zero
    var undefinedToZero = function(scoreDataPoint) {
        var retVal = scoreDataPoint;
        if (retVal === undefined) {
            retVal = 0;
        }
        return retVal;
    ...
    var processMetaData = function(scorefieldArray, starMetaData, starOrigin, records, namespacePrefix) {
        for (var scoreIndex = 0; scoreIndex < scorefieldArray.length; scoreIndex++) starMetaData[scorefieldArray[scoreIndex].trim() + "#" + "0"] = starOrigin;
    ...
    var mergeStarData = function(records, starCurrentCoordinate, scorefieldArray, 
        ...
        records.forEach(function(record) {
            ...
            for (var scoreIndex = 0; scoreIndex < scorefieldArray.length; scoreIndex++){
				starCurrentCoordinate += starMetaData[scorefieldArray[scoreIndex].trim() + "#" + undefinedToZero(record.get(namespacePrefixObject + scorefieldArray[scoreIndex].trim()))] + " ";
			}
            // set the starId
            ...
            if (colourIdx >= colours.length - 1 && colourIdx % colours.length === 0) colourIdx = 0
        })
		if(window.$Label===undefined){
			window.$Label = window.$Label || {};
			$Label.StarGraphOrderLabel = false;
		}
		if($Label.StarGraphOrderLabel==true){
			colourIdx = 0;
			var i;
			for (i = starLineData.length-1; i >= 0; i--) {
				starLineData[i].starColor=colours[colourIdx++];
    ...
    var makeSVGElement = function(svgTag, imageURI) {
        ...
        starImage.onload = function() {
            if (detectBrokenBrowsers() === !0) {// detects IE
    ...
    var makePolygons = function(data, svgTag, IdOfRecord) {
        ...
        $j.each(data, function(idx, starObject) {
            ...
            if (starObject.starId !== IdOfRecord && idx !== data.length - 1) $j("#" + starObject.starId).hide()
        })
    };
    ...
    var detectBrokenBrowsers = function() {
        ...
        if (navigator.appName === "Microsoft Internet Explorer" || navigator.appName == "Netscape") {
            ...
            if (regExpress.exec(navigator.userAgent) !== null) retVal = !0
        }
        return retVal
    };
